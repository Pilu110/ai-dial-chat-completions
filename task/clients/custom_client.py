...
class DialClient(BaseClient):
    ...
    def get_completion(self, messages: list[Message]) -> Message:
        ...
        requests_data = {"messages": [ msg.to_dict() for msg in messages]}

        response = requests.post(url=self._endpoint, headers=headers, json=requests_data)
    ...
    async def stream_completion(self, messages: list[Message]) -> Message:
        ...
        headers = {"api-key": self._api_key, "Content-Type": "application/json"}
        requests_data = {"stream": True, "messages": [ msg.to_dict() for msg in messages]}

        contents = []

        async with aiohttp.ClientSession() as session:
            async with session.post(url=self._endpoint, headers=headers, json=requests_data) as response:
